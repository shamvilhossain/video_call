<?php
 ...
 class VideoRoomsApiController extends Controller
 {
    ...
    public function createJoinRoom(Request $request)  // for initiator (create+join)
    {
        $list_user_type = array('doctor','service');

        if(in_array($request->user_type, $list_user_type))
        {
            $client = new Client($this->sid, $this->token);
            ...
            $data=['statusCode' => 400,'accessToken' =>null, 'roomName' =>null, 'msg' => 'User Type Not Allowed'];
            return response()->json($data);
        }
         
    }

    public function createRoom(Request $request)  // for initiator (create only)
    {
        $list_user_type = array('doctor','service');
        ...
        if(in_array($request->user_type, $list_user_type))
        {
            ...
            // insert in room tbl
                ...
                $data=['statusCode' => 200,'accessToken' => null, 'roomName' => $request->roomName, 'msg' => 'room created'];
                return response()->json($data); 
            }else{
                $data=['statusCode' => 400,'accessToken' =>null, 'roomName' =>null, 'msg' => 'failed'];
                return response()->json($data); 
     ...
     {
        $list_user_type = array('doctor','patient','service');

        if(in_array($request->user_type, $list_user_type))
        {
            $client = new Client($this->sid, $this->token);
            $exists = $client->video->v1->rooms->read(["status" => "in-progress", "uniqueName" => $request->roomName]);
            ...
            if (!empty($exists)) {
                ...
                $data=['statusCode' => 400,'accessToken' => null, 'roomName' => null, 'msg' => 'room not exists'];
                return response()->json($data); 
            }
        }else{
            $data=['statusCode' => 400,'accessToken' =>null, 'roomName' =>null, 'msg' => 'User Type Not Allowed'];
     ...
     public function disconnectRoom(Request $request)
     {
        ...
        $client = new Client($this->sid, $this->token);
        //$room = $client->video->v1->rooms($request->roomName)->fetch();  // only for in-progress 
        ...
        if(empty($rooms)){
            $data=['statusCode' => 400, 'msg' => 'Invalid Request'];
            return response()->json($data); 
        }else{
